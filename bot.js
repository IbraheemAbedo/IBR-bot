...
// ============== [نظام حفظ المستخدمين في قاعدة بيانات] ==============
...
initDatabase().then(success => {
  if (success) {
    console.log('📊 نظام حفظ المستخدمين مفعل');
  }
});
// ============== [وظائف الملفات] ==============
function loadData() {
...
bot.start(async (ctx) => {
  ...
  // الكود القديم يبقى كما هو (بدون تغيير)
...
bot.command('admin', async (ctx) => {
  ...
  try {
    ...
    message += `\n📊 *لتصدير البيانات:*\n`;
    message += `/export - لتصدير قائمة المستخدمين\n`;
    message += `/stats_db - لإحصائيات مفصلة`;
...
bot.command('export', async (ctx) => {
  ...
  try {
    const users = await getRecentUsers(1000); // كل المستخدمين
    
    if (users.length === 0) {
      return ctx.reply('📭 لا يوجد بيانات للتصدير.');
    }
    
    let csv = 'ID,Username,First Name,Last Name,Join Date\n';
    
    users.forEach(user => {
      csv += `${user.user_id},${user.username || ''},${user.first_name || ''},${user.last_name || ''},${user.join_date}\n`;
    });
...
bot.command('stats_db', async (ctx) => {
  ...
  try {
    ...
    // عدد المستخدمين النشطين (آخر أسبوع)
...
bot.action(/page_(\d+)/, async (ctx) => {
  ...
  // إعادة تنفيذ الأمر مع رقم الصفحة
